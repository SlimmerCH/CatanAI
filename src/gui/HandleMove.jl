...
function handle_move(move)
    ...
    if move["type"] == "discard"
        discard_amounts = move["cards"]
        trade_obj::Vector{UInt8} = UInt8[]
        for (i, amount) in enumerate(discard_amounts)
            if amount > 0
                element::UInt = amount << 4 | i
                push!(trade_obj, element)
            end
        end
        @show trade_obj
        move_obj = CatanBoard.Move(trade_obj, UInt8[], UInt8(0))
        validate_and_play(move_obj, true)
    end
end
